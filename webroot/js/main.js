...
window.vars = {
    "logLevel" : 8,              // Уровень журналирования в консоль (общий для всех тэгов (уровень журналирования тега приоритетней))
...
w().logger = new Logger(8, true, false, true, true);      // Создаём логер
var log = w().logger;
...
w().user = {
    ...
    "numberInGame"  : null,         // Номер игрока в игровой сессии
    "onSessionsList": null,         // Ссылка на обработчик сообщений с обновлением списка игровых сессий (используется на этапе entrance)
    "onGameMessage" : null,         // Ссылка на обработчик сообщений от игрового вертикла (приходят на адрес клиента)
    "scale"         : 100            // Масштаб (для рисования)
};
