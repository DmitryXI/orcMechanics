...
window.vars = {
    "logLevel" : 4,              // Уровень журналирования в консоль
...
w().logger = new Logger(8, false, true, true);      // Создаём логер
var log = w().logger;
log.addTag("msg");            // Добавляем тэг для фильтрации журналов сообщений        (сообщения)
log.addTag("form");           // Добавляем тэг для фильтрации журналов работы с формами (события HTML-форм)
log.addTag("func");           // Добавляем тэг для фильтрации обращения к функциям      (события связанные с функциями)
log.addTag("data");           // Добавляем тэг для фильтрации обращения к функциям      (события связанные с переменными и данными)
